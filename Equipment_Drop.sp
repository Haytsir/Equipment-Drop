...
#define CSGO_HEGRENADE	14 // HE Grenade	
#define CSGO_FLASH		15 // Flashbang
#define CSGO_SMOKE		16 // Smoke Grenade
#define CSGO_INCENDIARY	17 // Incendiary Grenade
#define CSGO_MOLOTOV	17 // Molotov Cocktail
#define	CSGO_DECOY		18 // Decoy Grenade
...
public OnPluginStart()
{
	...
	if(g_Game != Engine_CSS && g_Game != Engine_CSGO)
	{
		SetFailState("This Plugin only works on Counter-Strike: Source and Counter-Strike: Global Offensive.");
	}
	
	CreateConVar("sm_equipment_drop_version", PLUGIN_VERSION, "Equipment Drop Version", FCVAR_DONTRECORD|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public Action EventHook_RoundFreezeEnd(Event event, const char[] Name, bool dontBroadcast)
{
...
public Action Command_Drop(client, const String:command[], argc)
{
	...
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		new nadeSlot = -1;
		if(StrEqual("knife", sz_Classname[7]) && GetConVarInt(ED_Allows_Knife) == 1)
		{
			CS_DropWeapon(client, weapon, true, true);
		//	SDKHooks_DropWeapon(client, weapon);
			return Plugin_Handled;
		}
		if(StrEqual("taser", sz_Classname[7]) && GetConVarInt(ED_Allows_Zeus) == 1 && g_Game == Engine_CSGO) // CS:GO Only (Zeus x27)
		{
		...
		else if(StrEqual("hegrenade", sz_Classname[7]) && GetConVarInt(ED_Allows_Hegrenade) == 1)
			nadeSlot = g_Game == Engine_CSS ? CSS_HEGRENADE : g_Game == Engine_CSGO ? CSGO_HEGRENADE : -1; // HE Grenade
		else if(StrEqual("flashbang", sz_Classname[7]) && GetConVarInt(ED_Allows_Flashbang) == 1)
			nadeSlot = g_Game == Engine_CSS ? CSS_FLASH : g_Game == Engine_CSGO ? CSGO_FLASH : -1; // Flashbang
		else if(StrEqual("smokegrenade", sz_Classname[7]) && GetConVarInt(ED_Allows_Smokegrenade) == 1)
			nadeSlot = g_Game == Engine_CSS ? CSS_SMOKE : g_Game == Engine_CSGO ? CSGO_SMOKE : -1; // Smoke Grenade
		else if(StrEqual("incgrenade", sz_Classname[7]) && GetConVarInt(ED_Allows_Incgrenade) == 1 && g_Game == Engine_CSGO) // CS:GO Only
			nadeSlot = CSGO_INCENDIARY; // Incendiary Grenade
		else if(StrEqual("molotov", sz_Classname[7]) && GetConVarInt(ED_Allows_Molotov) == 1 && g_Game == Engine_CSGO) // CS:GO Only
			nadeSlot = CSGO_MOLOTOV; // Molotov Cocktail
		else if(StrEqual("decoy", sz_Classname[7]) && GetConVarInt(ED_Allows_Decoy) == 1 && g_Game == Engine_CSGO) // CS:GO Only
		...
		if(nadeCount > 0)
		{
			if((nSequence != 5 && g_Game == Engine_CSS) || (nSequence != 2 && g_Game == Engine_CSGO)) // Block nade duplicating by dropping a nade when player is uping his arm (right before throwing).
			{
				CS_DropWeapon(client, weapon, true, true);
			//	SDKHooks_DropWeapon(client, weapon);
...
{
	new nadeOffs = FindDataMapInfo(client, "m_iAmmo") + (slot * 4);
